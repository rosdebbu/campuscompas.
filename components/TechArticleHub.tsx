...
const WriteArticleModal: React.FC<{ onClose: () => void; onPublish: (article: Omit<TechArticle, 'id' | 'author' | 'date' | 'claps'>) => void; }> = ({ onClose, onPublish }) => {
    ...
    return (
        <BaseModal onClose={onClose}>
            <h2 className="text-2xl font-bold text-rose-900 dark:text-rose-100 mb-6 text-center">Write New Article</h2>
            <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto pr-2">
                <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Article Title" required className="w-full p-3 rounded-lg bg-lime-100 dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-rose-400" />
                <textarea value={content} onChange={e => setContent(e.target.value)} rows={8} placeholder="Write your content here... (Markdown is supported)" required className="w-full p-3 rounded-lg bg-lime-100 dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-rose-400"></textarea>
                <input type="text" value={tags} onChange={e => setTags(e.target.value)} placeholder="Tags (comma-separated, e.g., React, Python)" className="w-full p-3 rounded-lg bg-lime-100 dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-rose-400" />
                <input type="url" value={repoUrl} onChange={e => setRepoUrl(e.target.value)} placeholder="Optional: GitHub Repo URL" className="w-full p-3 rounded-lg bg-lime-100 dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-rose-400" />
                <button type="submit" className="w-full py-3 bg-rose-400 text-white font-semibold rounded-lg shadow hover:bg-rose-50 dark:bg-slate-8000 transition-colors">Publish Article</button>
            </form>
        </BaseModal>
    );
};
...
const ReadArticleModal: React.FC<{ article: TechArticle; onClose: () => void; }> = ({ article, onClose }) => (
    <BaseModal onClose={onClose}>
        <div className="overflow-y-auto pr-4">
            <h2 className="text-3xl font-bold text-rose-900 dark:text-rose-100">{article.title}</h2>
            <div className="flex items-center my-4">
                <img src={article.author.avatarUrl} alt={article.author.name} className="w-10 h-10 rounded-full mr-3" />
                <div>
                    <p className="font-semibold text-rose-800 dark:text-rose-200">{article.author.name}</p>
                    <p className="text-sm text-rose-500 dark:text-rose-400">{article.date}</p>
                </div>
            </div>
            {article.repoUrl && (
            ...
            <div className="prose max-w-none text-rose-800 dark:text-rose-200">
...
const ArticleCard: React.FC<{ article: TechArticle; onReadMore: () => void; onClap: () => void; }> = ({ article, onReadMore, onClap }) => (
    <div className="bg-white dark:bg-slate-900 rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300 flex flex-col border border-lime-200 dark:border-slate-700">
        <img src={article.imageUrl} alt={article.title} className="w-full h-48 object-cover" />
        <div className="p-6 flex flex-col flex-grow">
            <div className="flex-grow">
                <h3 className="text-xl font-bold text-rose-900 dark:text-rose-100">{article.title}</h3>
                <div className="flex items-center my-3">
                    <img src={article.author.avatarUrl} alt={article.author.name} className="w-8 h-8 rounded-full mr-2" />
                    <p className="text-sm text-rose-600 dark:text-rose-300">{article.author.name} • {article.date}</p>
                </div>
            ...
            <div className="mt-4 pt-4 border-t border-lime-200 dark:border-slate-700 flex justify-between items-center">
                <button onClick={onClap} className="flex items-center gap-2 text-rose-600 dark:text-rose-300 font-semibold">
                    <ClapIcon /> {article.claps}
                </button>
                <button onClick={onReadMore} className="font-semibold text-rose-500 dark:text-rose-400 hover:text-rose-700 transition-colors">Read More &rarr;</button>
            </div>
        </div>
    </div>
);
            ...
            <div className="container mx-auto px-4">
                <div className="text-center mb-12">
                    <div className="flex justify-center items-center mb-4">
                        <ArticleHubIcon />
                        <h2 className="text-3xl md:text-4xl font-extrabold text-rose-900 dark:text-rose-100">Tech Article Hub</h2>
                    </div>
                    ...
                    <button onClick={() => setIsWriteModalOpen(true)} className="mt-6 inline-flex items-center px-6 py-3 bg-rose-400 text-white font-semibold rounded-full shadow-lg hover:bg-rose-50 dark:bg-slate-8000 transition-colors transform hover:scale-105">
