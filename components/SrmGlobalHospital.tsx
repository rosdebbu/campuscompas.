...
const ClipboardListIcon: React.FC = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>;
const CubeIcon: React.FC = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-14L4 7v10l8 4" /></svg>;
...
const ActionCard: React.FC<ActionCardProps> = ({ icon, title, description, onClick, href, isExternal }) => {
    const content = (
        <div className="bg-white rounded-2xl shadow-lg p-6 text-center flex flex-col items-center justify-start border hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full">
            <div className="bg-lime-100 p-4 rounded-full mb-4">
                {icon}
            </div>
            <h3 className="text-xl font-bold text-rose-900">{title}</h3>
            <p className="text-sm text-rose-600 mt-2 flex-grow">{description}</p>
        </div>
    );
                ...
                return (
                    <div className="space-y-12">
                         <div className="text-center">
                            ...
                            <p className="text-lg text-rose-600 max-w-2xl mx-auto">Your health partner on campus. Find specialists, get immediate help, and access key resources.</p>
                        </div>
                        ...
                        <div className="max-w-3xl mx-auto">
                             <h3 className="text-2xl font-bold text-rose-900 text-center mb-6">Common Services</h3>
                             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {commonServices.map(service => (
                                    <div key={service.name} className="bg-white p-4 rounded-lg shadow-sm border border-lime-200 text-center">
                                        <service.icon />
                                        <p className="font-semibold text-rose-800 text-sm mt-2">{service.name}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
