...
const CalendarIcon: React.FC = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-rose-500 dark:text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>;
...
const InfoCard: React.FC<InfoCardProps> = ({ icon, title, children }) => (
    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm p-6 border border-lime-200 dark:border-slate-700 h-full flex flex-col">
        <div className="flex items-center mb-4">
            {icon}
            <h3 className="text-xl font-bold text-rose-900 dark:text-rose-100 ml-3">{title}</h3>
        </div>
            ...
            <div className="container mx-auto px-4">
                <div className="text-center mb-12">
                    <div className="flex justify-center items-center mb-4">
                        <DigestIcon />
                        <h2 className="text-3xl md:text-4xl font-extrabold text-rose-900 dark:text-rose-100">Your Weekly Digest</h2>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-8">
                        <InfoCard icon={<CalendarIcon />} title="My Schedule">
                            {mySchedule.map(item => (
                                <div key={item.id} className="bg-rose-50 dark:bg-slate-800 p-3 rounded-lg">
                                    <p className="font-semibold text-rose-800 dark:text-rose-200">{item.title}</p>
                                    <p>{item.date} @ {item.time}</p>
                                    <p className="text-xs text-rose-500 dark:text-rose-400">{item.location}</p>
                                </div>
                            ))}
                        </InfoCard>
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-lime-100 dark:bg-slate-800 rounded-xl shadow-sm p-6 border border-lime-300 text-center sticky top-24">
                            <h3 className="text-2xl font-bold text-rose-900 dark:text-rose-100">Stay in the Loop!</h3>
                            ...
                            <form className="flex flex-col gap-3" onSubmit={e => { e.preventDefault(); alert('Subscribed!'); }}>
                                <input type="email" placeholder="you@srmist.edu.in" required className="w-full py-3 px-4 rounded-lg text-rose-800 dark:text-rose-200 placeholder-rose-400 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-400 border border-transparent focus:border-rose-300" />
                                <button type="submit" className="w-full py-3 bg-rose-400 text-white font-semibold rounded-lg shadow-md hover:bg-rose-50 dark:bg-slate-8000 transition-colors">
