        ...
        return (
             <>
                ...
                <div className="flex justify-between items-center border-b border-rose-200 pb-4 mb-4">
                    <div>
                         <button onClick={handleBackClick} className="text-sm text-rose-500 dark:text-rose-400 hover:text-rose-700 mb-1">
                            &larr; Back to Main Building
                        </button>
                        <h2 id="building-detail-title" className="text-2xl font-bold text-rose-900 dark:text-rose-100">{selectedHostel.name}</h2>
                    </div>
                    <button onClick={onClose} className="text-rose-400 hover:text-rose-600 dark:text-rose-300 transition-colors" aria-label="Close building details">
                        <CloseIcon />
                    </button>
                </div>
                ...
                <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 overflow-hidden md:overflow-y-auto">
                            ...
                            {floorKeys.map(floorKey => (
                                <li key={floorKey}>
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2 rounded-lg transition-colors duration-200 ${activeFloor === floorKey ? 'bg-rose-400 text-white font-semibold' : 'text-rose-700 hover:bg-lime-100 dark:bg-slate-800'}`}
                                    >
                                        {floorKey}
                                    </button>
                                </li>
                    ...
                    {/* Room List */}
                    <div className="md:col-span-2 bg-white dark:bg-slate-900 rounded-xl p-6 overflow-y-auto">
                        {activeFloor ? (
                            <div>
                                <h3 className="text-xl font-bold text-rose-900 dark:text-rose-100 mb-4">Rooms on {activeFloor}</h3>
                                <div className="flex flex-wrap gap-2">
                                    {selectedHostel.floorData[activeFloor].map((room, index) => (
                                        <div key={index} className="bg-lime-50 dark:bg-slate-950 text-rose-800 dark:text-rose-200 font-mono text-sm px-3 py-1.5 rounded-md border border-lime-200 dark:border-slate-700">
                                            {room}
                                        </div>
                                    ))}
                                </div>
                             ...
                             <div className="flex flex-col items-center justify-center h-full text-center">
                                ...
                                <h3 className="text-xl font-semibold text-rose-800 dark:text-rose-200">Select a floor</h3>
                                <p className="text-rose-600 dark:text-rose-300 max-w-xs mt-1">Choose a floor to see the list of rooms.</p>
                            </div>
                        )}
                    </div>
                </div>
    ...
    const renderMainBuildingView = () => {
        ...
        return (
            <>
                {/* Header */}
                <div className="flex justify-between items-center border-b border-rose-200 pb-4 mb-4">
                    <h2 id="building-detail-title" className="text-2xl font-bold text-rose-900 dark:text-rose-100">{building.title}</h2>
                    <button onClick={onClose} className="text-rose-400 hover:text-rose-600 dark:text-rose-300 transition-colors" aria-label="Close building details">
                        <CloseIcon />
                    </button>
                </div>
                ...
                <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 overflow-hidden md:overflow-y-auto">
                        ...
                        <div>
                         ...
                         {building.buildingDetails?.otherInfo && (
                            <div className="mt-6 pt-6 border-t border-rose-200">
                                <h3 className="text-lg font-semibold text-rose-800 dark:text-rose-200 mb-3">{building.buildingDetails.otherInfo.title}</h3>
                                <ul className="space-y-2">
                                    {building.buildingDetails.otherInfo.items.map((hostel, index) => (
                                         <li key={index}>
                                            <button 
                                                onClick={() => handleHostelClick(hostel)}
                                                className="w-full text-left text-rose-700 p-3 rounded-lg hover:bg-lime-100 dark:bg-slate-800 transition-colors duration-200 flex items-start"
                                            >
                    ...
                    <div className="md:col-span-2 bg-white dark:bg-slate-900 rounded-xl p-6 overflow-y-auto">
                        {activeFloor ? (
                            <div>
                                <h3 className="text-xl font-bold text-rose-900 dark:text-rose-100 mb-4">Details for {activeFloor}</h3>
                                <ul className="space-y-3">
                                    {building.buildingDetails!.floorData[activeFloor].map((person, index) => (
                                        <li key={index} className="bg-lime-50 dark:bg-slate-950 p-3 rounded-md text-rose-800 dark:text-rose-200 border border-lime-200 dark:border-slate-700">
                                            {person}
                                        </li>
                                    ))}
                                </ul>
                            ...
                            <div className="flex flex-col items-center justify-center h-full text-center">
                                ...
                                <h3 className="text-xl font-semibold text-rose-800 dark:text-rose-200">Select a floor</h3>
                                <p className="text-rose-600 dark:text-rose-300 max-w-xs mt-1">Choose a floor from the list on the left to see the details.</p>
                            </div>
                        )}
                    </div>
                </div>
...
const galleryItems: GalleryItem[] = [
    { 
        ...
        imageUrl: '/images/gallery/main_campus_building_1775165717748.png',
        buildingDetails: {
            floorCount: 8,
            floorData: {
    ...
    { 
        ...
        imageUrl: '/images/gallery/central_library_1775165735784.png',
        buildingDetails: {
            floorCount: 14,
            floorData: {
    ...
    { 
        ...
        imageUrl: '/images/gallery/tech_park_1775165753691.png',
        buildingDetails: {
            floorCount: 15,
            floorData: {
    ...
    { 
        ...
        imageUrl: '/images/gallery/sports_complex_1775165771956.png',
        buildingDetails: {
            floorCount: 4,
            floorData: {
    ...
    { 
        ...
        imageUrl: '/images/gallery/student_hostels_1775165785864.png',
        buildingDetails: {
    ...
    { 
        ...
        imageUrl: '/images/gallery/tp_auditorium_1775165804156.png',
        isFeatured: true,
