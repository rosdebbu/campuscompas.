...
const mockIssues: OpenSourceIssue[] = [
    { id: 1, repoName: 'awesome-for-beginners/awesome-for-beginners', repoUrl: 'https://github.com/awesome-for-beginners/awesome-for-beginners', title: 'Add a new project to the Python list', url: 'https://github.com/awesome-for-beginners/awesome-for-beginners/issues/1', language: 'Python', labels: [{ name: 'good first issue', color: 'bg-teal-200 text-teal-800' }, { name: 'help wanted', color: 'bg-rose-200 text-rose-800 dark:text-rose-200' }] },
    ...
    { id: 4, repoName: 'golang/go', repoUrl: 'https://github.com/golang/go', title: 'Add example for strings.Builder', url: 'https://github.com/golang/go/issues/4', language: 'Go', labels: [{ name: 'documentation', color: 'bg-blue-200 text-blue-800' }, { name: 'help wanted', color: 'bg-rose-200 text-rose-800 dark:text-rose-200' }] },
    ...
    { id: 6, repoName: 'denoland/deno', repoUrl: 'https://github.com/denoland/deno', title: 'Update dependencies in `std` module', url: 'https://github.com/denoland/deno/issues/6', language: 'TypeScript', labels: [{ name: 'help wanted', color: 'bg-rose-200 text-rose-800 dark:text-rose-200' }] },
];
...
const IssueCard: React.FC<{ issue: OpenSourceIssue }> = ({ issue }) => (
    <div className="bg-white dark:bg-slate-900 p-4 rounded-lg border border-lime-200 dark:border-slate-700 hover:shadow-md hover:border-rose-300 transition-all duration-300">
        <a href={issue.repoUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-rose-600 dark:text-rose-300 hover:text-rose-800 dark:text-rose-200 group inline-flex items-center">
            {issue.repoName} <ExternalLinkIcon />
        </a>
        <h3 className="font-bold text-rose-900 dark:text-rose-100 mt-1">
            <a href={issue.url} target="_blank" rel="noopener noreferrer" className="hover:underline">{issue.title}</a>
        </h3>
        <div className="flex items-center flex-wrap gap-2 mt-3">
            <span className="text-xs font-semibold bg-gray-200 text-gray-800 dark:text-gray-200 px-2 py-1 rounded-full">{issue.language}</span>
        ...
        <a href={issue.url} target="_blank" rel="noopener noreferrer" className="inline-block mt-4 w-full text-center py-2 bg-rose-400 text-white font-semibold rounded-lg hover:bg-rose-50 dark:bg-slate-8000 transition-colors">
            Contribute
        </a>
    </div>
);
                ...
                <div className="overflow-y-auto pr-4 space-y-4">
                    {steps.map(step => (
                        <div key={step.title} className="bg-lime-50 dark:bg-slate-950 p-4 rounded-lg border border-lime-200 dark:border-slate-700">
                            <h3 className="font-bold text-rose-800 dark:text-rose-200">{step.title}</h3>
                ...
                <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mb-8">
                    {languages.map(lang => (
                        <button
                            ...
                            className={`px-4 py-2 text-sm font-semibold rounded-full transition-all duration-300 ${selectedLanguage === lang ? 'bg-rose-400 text-white shadow-md' : 'bg-white dark:bg-slate-900 text-rose-700 hover:bg-lime-200'}`}
                        >
                            {lang}
                        </button>
                    ))}
